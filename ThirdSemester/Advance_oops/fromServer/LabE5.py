...
class Employee:
    ...
    def add(self, slno, name, address, empcode, dob, age, mobile, status, designation):
        v = self.cur.execute('''Insert into Emp values(%d,'%s','%s','%s',%d,%d,%d,'%s','%s')''' % (
            int(slno), name, address, empcode, int(dob), int(age), int(mobile), status, designation))
    ...
    def update(self, stat, sln):
        self.cur.execute(
            '''Update Emp set status='%s' where slno=%d''' % (stat, int(sln)))
...
while True:
    ...
    if ch == 1:
        ...
        e.add(slno, name, address, empcode, dob,
              age, mobile, status, designation)
